...
int main(int argc, char *argv[])
{
	...
	output.open(NameFileWithTime("ldata"));
	outFile.open(NameFileWithTime("lraw"));
	outangle.open(NameFileWithTime("langle"));
	path.open(NameFileWithTime("pathXY"));
	outTime.open(NameFileWithTime("lTime"));
	cv::Point tmpPoint;
	...
	while (runFlag)
	{
		...
		for (int i = 0; i < totalPoint; i++)
		{
...
#endif
			{
				...
				// outFile << i << ' ' << tmpPoint.x << ' ' << tmpPoint.y << endl;
			}
		}
		// //复制场地图像信息
