...
void MatchMap(void)
{
    ...
    for (int lNum = 0; lNum < allCalculateLine.size(); lNum++)
    {
        ...
        for (int wNum = 0; wNum < wallLine.size(); wNum++)
        {
            ...
            if (wNum == 9 && robot.rbotPos.y > 8000) // 9号墙的点有点问题导致x在要取箭时会有先正矫正100在负矫正100的现象，先暂时用7号墙来矫正x坐标
            {
                continue;
            }
            //如果找到匹配的墙则将其标号存入直线参数
...
#ifdef TRMODE
    // TR在取箭之后射箭时进行矫正坐标
    bool corFlag1 = (robot.rbotPos.x > 1300 && robot.rbotPos.y < 6200 && robot.rbotPos.y > 3200);
    // TR快要取箭时进行矫正坐标
