...
void ReceiveData(int fd)
{
    ...
    ppsOut.open(NameFileWithTime1("ppsOut"));
            ...
            case 4:
            {
                ...
                if (ch == 'a')
                {
                    ...
                    if (ppsReadyFlag)
                    {
                        ...
                        {
                            ppsOut<<"origin"<<" "<<rData.dataF[0]<<" "<<rData.dataF[1]<<" "<<rData.dataF[2]<<" "<<rData.dataF[3]<<" "<<rData.dataF[4]<<" "<<rData.dataF[5]<<endl;
                            ...
                            if (fabs(ppsData.speedX - lastPPSData.speedX) > 1000 || fabs(ppsData.speedY - lastPPSData.speedY) > 1000)
                            {
                                ...
                                ppsOut << "wwwwwwwww" << endl;
                                count = 0;
                                break;
                            }
                            // if ((robot.posData.size() != 0) && proStamp>200)
                            // {
...
//     return 0;
// }
char *NameFileWithTime1(string fileName)
{
	char str1[20];
	char str2[20];
