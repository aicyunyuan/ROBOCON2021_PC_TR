...
void OutputVel2Wheel(float vel, float direction, float omega)
{
	...
	// if(GetX() < 850.0f && GetY() > 11350.0f && (gRobot.walkStatus == goForFirstPath || gRobot.walkStatus ==  goForThirdPath || gRobot.walkStatus ==  goForFifthPath || gRobot.walkStatus ==  goForEleventhPath))
	// {
	// 	vel = 1050.0f;	
	// 	// if(fabs(GetAngle()) > 178.5 )
	// 	// {
	// 	// 	direction = 180.0f;
	// 	// }	
	// }

	// if(GetY() > 11280.0f && gRobot.walkStatus == goForFirstPath)
	// {
	// 	direction = 180.0f;
	// 	vel = 0.0f;
	// 	omega = 0.0f;
	...
	// if(GetX() < 1180.0f && GetY() > 11380.0f && gRobot.walkStatus == goForFirstPath)
	// {
	// 	omega = 0.0f;
	// }
...
void WheelLockTransform(float oneTargetDir, float twoTargetDir, float thrTargetDir)
{
	wheel_t outputVel = {0.0f};
	//计算每个轮子的和速度大小和方向
	outputVel.one = CalcWheelSpeed(0.0f , (GetAngle() + oneTargetDir) , 0.0f , ONE_VERTICAL_ANG , GetAngle());
	
	outputVel.two = CalcWheelSpeed(0.0f , (GetAngle() + twoTargetDir) , 0.0f , TWO_VERTICAL_ANG , GetAngle());
	
	outputVel.three = CalcWheelSpeed(0.0f , (GetAngle() + thrTargetDir) , 0.0f , THREE_VERTICAL_ANG , GetAngle());
