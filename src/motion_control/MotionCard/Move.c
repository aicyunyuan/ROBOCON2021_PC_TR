...
void VelControl(robotVel_t actVel)
{
	...
	if(GetX() < 800.0f && GetY() > 10000.0f && (gRobot.walkStatus ==  goForThirdPath || gRobot.walkStatus ==  goForFifthPath))
	{
		OutputVel2Wheel(1180.f, targetVel.direction , 0.f);
	}
	else if(GetX() < 800.0f && GetY() > 10000.0f && gRobot.walkStatus == goForFirstPath)
	{
		OutputVel2Wheel(1100.f, targetVel.direction , 0.f);
	}
	else if(GetX() < 750.0f && GetY() > 10000.0f && gRobot.walkStatus == goForEleventhPath)
	{
		OutputVel2Wheel(1130.f, targetVel.direction , 0.f);
	}
	else
	{
...
//void ThreeWheelVelControl(float speed, float direction, float rotationVell)//void ThreeWheelVelControl(float speed, float direction, float rotationVell)
