...
void WaitPpsPrepare(void)
{
    ...
    printf("pps reset %d %d %d \n",ppsDataInit[0],ppsDataInit[1],ppsDataInit[2]);
...
void PpsDataRecognize(void)
{
    ...
    if(recPpsData[0] == 'A' && recPpsData[1] == 'T' && recPpsData[MC_LIDAR_BUF_LENGTH - 2] == 't' && recPpsData[MC_LIDAR_BUF_LENGTH - 1] == 'a')
    {
        ...
        {
            if(ppsTalkMontionCtrlOk == 0)
            {
                printf("pps read ");
                for(int ppsTestCnt = 0; ppsTestCnt < MC_LIDAR_BUF_LENGTH;ppsTestCnt ++)
                {
                    printf("%d ",(int)recPpsData[ppsTestCnt]);
                }
                printf("\n");
            }
